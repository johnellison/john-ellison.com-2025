...
function StatCounter({ target, suffix, label }: { target: number; suffix: string; label: string }) {
  ...
  return (
    <div className="flex flex-col items-center justify-center p-4 bg-white/[0.03] rounded-xl border border-white/5 hover:border-violet-500/20 transition-colors duration-300">
      {/* Starting with 0 to match server-side render and prevent hydration mismatch */}
      <span ref={counterRef} className="type-2xl font-bold text-white mb-1 tabular-nums">
        0{suffix}
      </span>
      <span className="type-xs font-semibold uppercase tracking-widest text-violet-300/80">
        {label}
      </span>
    </div>
  );
